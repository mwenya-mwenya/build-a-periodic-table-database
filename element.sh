...
ELEMENT_QUERY(){ #joins tables to get correct fields and assigns read variables to display correct format
ATOMIC_NUMBER_QUERY=$($PSQL "SELECT * FROM elements JOIN properties USING(atomic_number) JOIN types USING(type_id) WHERE atomic_number = $ELEMENT_QUERY_NUMBER ")
echo "$ATOMIC_NUMBER_QUERY" | while IFS="|" read TYPE_ID ATOMIC_NUMBER ATOMIC_SYMBOL NAME ATOMIC_MASS MELTING_POINT BOILING_POINT TYPE;
do
